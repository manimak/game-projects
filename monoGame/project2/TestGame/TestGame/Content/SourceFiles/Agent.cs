...
public abstract class Agent : MovableModel3D {
  ...
  public Object3D AgentObject {
   ...
   public override void Update(GameTime gameTime) {
            foreach (Treasures treasure in stage.AllTreasures)
            {
                if (Vector3.Distance(agentObject.Translation,treasure.Position) <= 200)
                {
                    treasure.Model = stage.Content.Load<Model>("diamandFlaged");
                    if (treasure.Flaged == false)
                    {
                        treasure.Flaged = true;
                        stage.NumUnTagedTereasure--;
                        treasureScore++;
                    }
      ...
      base.Update(gameTime);
            ...
            if (lerp)
            {
                stage.lerpSetSurfaceHeight(agentObject);
            }
            else
            {
            ...
            if (keyboardState.IsKeyDown(Keys.L) && !oldKeyboardState.IsKeyDown(Keys.L))
            {
                if (lerp)
                {
                    lerp = false;
