...
public class Stage : Game {
   ...
   protected Sensor sensor1 =null;
   protected Sensor sensor2 =null;
   ...
   protected List<Treasures> allTreasurs = new List<Treasures>();
   protected int[,] treasurePositions = { { 447, 457 }, { 410, 500 }, { 430, 430 }, { 450, 420 }, { 450, 430 } };
   protected int numUnTagedTereasure = 5;
   ...
   public int TerrainSize {
	...
	public TimeSpan Time {  // Update's GameTime
		get { return time; }}
    public List<Treasures> AllTreasures
    {
        get { return allTreasurs; }

    }
    public int[,] TreasurePositions
    {
        get { return treasurePositions; }
    }

    public int NumUnTagedTereasure
        {
            get { return numUnTagedTereasure; }
            set { numUnTagedTereasure = value; }
   ...
   public float surfaceHeight(float x, float z) {
      ...
      anObject3D.Translation = new Vector3( anObject3D.Translation.X, terrainHeight, 
       ...
       public void lerpSetSurfaceHeight(Object3D anObject3D)
       {
            float X = anObject3D.Translation.X;
            float Z = anObject3D.Translation.Z;
            Vector3 aPoint = new Vector3((int) X/spacing,terrain.surfaceHeight((int) X/spacing,(int) Z/spacing),(int) Z/spacing);
            Vector3 bPoint = new Vector3((int) Math.Ceiling(X/spacing),terrain.surfaceHeight((int) Math.Ceiling(X/spacing),(int) Z/spacing),(int) Z/spacing);
            Vector3 cPoint = new Vector3((int) X/spacing ,terrain.surfaceHeight((int) X/spacing,(int) Math.Ceiling(Z/spacing)), (int) Math.Ceiling(Z/spacing));
            float xY = Vector3.Lerp(aPoint, bPoint, (float)(X/spacing-aPoint.X)).Y - aPoint.Y;
            float zY = Vector3.Lerp(aPoint, cPoint, (float)(Z/spacing-aPoint.Z)).Y - aPoint.Y;
            anObject3D.Translation = new Vector3(anObject3D.Translation.X,aPoint.Y+xY+zY,anObject3D.Translation.Z);
       }
   ...
   protected override void LoadContent() {
      ...
      player = new Player(this,"Chaser",
         new Vector3(430 * spacing, terrain.surfaceHeight(430, 440), 440 * spacing),
         new Vector3(0, 1, 0), 0.78f, "model4");  // face looking diagonally across stage
      ...
      Components.Add(npAgent);
       ...
       Vector3 sensorPos = new Vector3(npAgent.AgentObject.Translation.X + 200, npAgent.AgentObject.Translation.Y, npAgent.AgentObject.Translation.Z + 200);
        sensor1 = new Sensor(this, "sensor1", sensorPos, new Vector3(0, 1, 0), 100f, "sensor");
        
        Components.Add(sensor1);
        sensor2 = new Sensor(this, "sensor2", sensorPos, new Vector3(0, 1, 0), 100f, "sensor");
        ...
        Components.Add(sensor2);
            ...
            for (int i = 0; i < treasurePositions.GetLength(0); i++)
        {
            int xPos = treasurePositions[i, 0];
            int zPos = treasurePositions[i, 1];
            Treasures treasures = new Treasures(this, "diamand", "diamand", xPos, zPos);
            allTreasurs.Add(treasures);
        }
        foreach (Treasures treasures in allTreasurs)
        {
