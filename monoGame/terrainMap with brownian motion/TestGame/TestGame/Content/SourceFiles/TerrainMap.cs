...
public class TerrainMap : Game {
   ...
   private Texture2D createHeightTexture() {
      ...
      float[,] heightMapNumber = new float[textureWidth, textureHeight];
      ...
      for (int x = 0; x < textureWidth; x++)
         for (int z = 0; z < textureHeight; z++) {
           ...
           // heightMap[x, z] = new Color(colorVec3);  // a color where r = g = b = the hieght value
            heightMapNumber[x, z] = (float)random.Next(3);
            }
        ...
        int radius = 33;
        int step = 11;
        int[,] center = new int[,] { { 100, 100 }, { 250, 120 }, { 400, 100 }, { 50, 256 } };
        int currentCenter, positionInHeightMapX, positionInHeightMapZ;
        
        for (int a = 0; a <= p; a++)
        {
            currentCenter = random.Next(0, nCenter);
            positionInHeightMapX = center[currentCenter, 0];
            positionInHeightMapZ = center[currentCenter, 1];

            for (int b = 0; b <= s; b++)
            {       
                    for (int x = (positionInHeightMapX - radius); x < (positionInHeightMapX + radius); x++)
                    {
                        
                        
                        for (int z = (positionInHeightMapZ - radius); z < (positionInHeightMapZ + radius); z++)
                        {
                           if(x<=0 || x>=512 || z<=0 || z>=512)
                            ...
                            heightMapNumber[x, z] = heightMapNumber[x, z] + 1.36f;
                            if(heightMapNumber[x, z] >= 255)
                            {
                                heightMapNumber[x, z] = 254
                                    ;
                            }
                    ...
                    positionInHeightMapX += (step * (random.Next(0,2) - 1));
                    positionInHeightMapZ += (step * (random.Next(0,2) - 1));
                    
                    
                    if (positionInHeightMapX > textureWidth || positionInHeightMapX<radius )
                    {
                        
                        positionInHeightMapX = random.Next(0,512);
                        positionInHeightMapZ = random.Next(0, 512);
                    }
                    if (positionInHeightMapZ > textureHeight-radius || positionInHeightMapZ<radius)
                    {
                        positionInHeightMapX = random.Next(0, 512);
                        positionInHeightMapZ = random.Next(0, 512);
                    }
        ...
        for (int x = 0; x < textureWidth; x++)
            {
                for (int z = 0; z < textureHeight; z++)
                {
                    ...
                    percentColor = heightMapNumber[x,z]/255.0f; // float version of byte value 
                    colorVec3 = new Vector3(percentColor, percentColor, percentColor);
                    heightMap[x, z] = new Color(colorVec3);  // a color where r = g = b = the hieght value
